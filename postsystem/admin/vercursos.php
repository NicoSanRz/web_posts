...
<?php
	...
	if(isset($_GET['delete'])){
		...
		// seleciona a imagem
		$seleciona = "SELECT * from tb_cursos WHERE id= :id_delete";
		try{
			...
			if($contar>0){
				...
				$arquivo = "../upload/cursos/" .$fotoDeleta;
				unlink($arquivo);
				
				
				// exclui o registo
				$seleciona = "DELETE from tb_cursos WHERE id=:id_delete";
				try{
					$result = $conexao->prepare($seleciona);
					$result->bindParam('id_delete',$id_delete, PDO::PARAM_INT);				
					$result->execute();
...
if(isset($_POST['palavra-busca'])){
	...
	$select = "SELECT * from tb_cursos WHERE titulo LIKE '%$busca%' ORDER BY id DESC LIMIT $inicio, $quantidade";	
}else{
	$select = "SELECT * from tb_cursos ORDER BY id DESC LIMIT $inicio, $quantidade";
}
...
?>           
                  <tr>
                    ...
                    <td><img src="../upload/cursos/<?php echo $mostra->imagem;?>" width="50"/></td>
                    <td> <?php echo limitarTexto($mostra->descricao, $limite=200)?> </td>
                    <td class="td-actions"><a href="home.php?acao=editar-cursos&id=<?php echo $mostra->id;?>" class="btn btn-small btn-success"><i class="btn-icon-only icon-edit"> </i></a>
                    
                    <a href="home.php?acao=ver-cursos&pg=<?php echo $pg;?>&delete=<?php echo $mostra->id;?>" class="btn btn-danger btn-small"  onClick="return confirm('Deseja realmente exluir o conteúdo?')"><i class="btn-icon-only icon-remove"> </i></a></td>
                  </tr>
<?php
}				
			}else{
