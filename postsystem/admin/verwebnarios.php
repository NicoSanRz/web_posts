...
<?php
	...
	if(isset($_GET['delete'])){
		...
		// seleciona a imagem
		$seleciona = "SELECT * from tb_webnarios WHERE id= :id_delete";
		try{
			...
			if($contar>0){
				...
				$arquivo = "../upload/webnarios/" .$fotoDeleta;
				unlink($arquivo);
				
				
				// exclui o registo
				$seleciona = "DELETE from tb_webnarios WHERE id=:id_delete";
				try{
					$result = $conexao->prepare($seleciona);
					$result->bindParam('id_delete',$id_delete, PDO::PARAM_INT);				
					$result->execute();
...
if(isset($_POST['palavra-busca'])){
	...
	$select = "SELECT * from tb_webnarios WHERE titulo LIKE '%$busca%' ORDER BY id DESC LIMIT $inicio, $quantidade";	
}else{
	$select = "SELECT * from tb_webnarios ORDER BY id DESC LIMIT $inicio, $quantidade";
}
...
?>           
                  <tr>
                    ...
                    <td><img src="../upload/webnarios/<?php echo $mostra->imagem;?>" width="50"/></td>
                    <td> <?php echo limitarTexto($mostra->descricao, $limite=200)?> </td>
                    <td class="td-actions"><a href="home.php?acao=editar-web&id=<?php echo $mostra->id;?>" class="btn btn-small btn-success"><i class="btn-icon-only icon-edit"> </i></a>
                    
                    <a href="home.php?acao=ver-web&pg=<?php echo $pg;?>&delete=<?php echo $mostra->id;?>" class="btn btn-danger btn-small"  onClick="return confirm('Deseja realmente exluir o post?')"><i class="btn-icon-only icon-remove"> </i></a></td>
                  </tr>
<?php
}				
			}else{
