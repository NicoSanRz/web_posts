            ...
            <div class="span12">	      		
	      		<div id="target-1" class="widget">	      			
	      			<div class="widget-content">	      				
			      		<h1>OVAN- Painel de Coberturas</h1>			      		
			      		<p>Painel de gerenciamento de Coberturas!
                </p>
...
$inicio = ($pg*$quantidade) - $quantidade;
  ...
  $get_portaria = "SELECT * FROM tb_coberturas";
  $contagem =1;
  try{
...
?>
        <tr>
          ...
          <td class="td-actions"><a href="home.php?acao=editar-coberturas&id=<?php echo $mostra->id;?>" class="btn btn-small btn-success"><i class="btn-icon-only icon-edit"> </i></a>
          
          <a href="home.php?acao=ver-coberturas&pg=<?php echo $pg;?>&delete=<?php echo $mostra->id;?>" class="btn btn-danger btn-small" onClick="return confirm('Deseja realmente exluir essa conta?')"><i class="btn-icon-only icon-remove"> </i></a></td>
        </tr> 
...
if($nivelLogado ==0){
  ...
  // seleciona a imagem
  $seleciona = "SELECT * from tb_coberturas WHERE id= :id_delete";
  try{
  ...
  if($contar>0){
    ...
    // exclui o registo
    $seleciona = "DELETE from tb_coberturas WHERE id=:id_delete";
    try{
      $result = $conexao->prepare($seleciona);
      $result->bindParam('id_delete',$id_delete, PDO::PARAM_INT);				
      $result->execute();
